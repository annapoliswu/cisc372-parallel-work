...
unsigned int *p, *q;
int left, right; // my left and right neighbors (ranks) neighbor procs
...
int main() {
  ...
  p = malloc((n_local+2)*sizeof(unsigned int)); //sep in each proc, local
  q = malloc((n_local+2)*sizeof(unsigned int));
  for (int i=0; i<n_local+2; i++) p[i] = q[i] = 0; //setsall array to 0
  if (OWNER(N) == myrank) p[1+LOCAL_INDEX(N)] = 1; //owner of middle
  left = n_local == 0 || first == 0 ? MPI_PROC_NULL : OWNER(first-1);
  right = n_local == 0 || first+n_local >= 2*N+1 ? MPI_PROC_NULL : OWNER(first+n_local);
  ...
  for (int i=0; i<N; i++) {
    print();
    exchange_ghosts(); //swap values to complete arrays
    update(); //all do individ calcs for 5 array nums and 
  }
  print();
  MPI_Finalize();
  free(p);
